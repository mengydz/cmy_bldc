...
uint8_t CAN_Init(CAN_TypeDef* CANx, CAN_InitTypeDef* CAN_InitStruct)
{
  ...
  /* Request initialisation 软件通过对CAN_MCR寄存器的INRQ位置1，来请求bxCAN进入初始化模式*/
  CANx->MCR |= MCR_INRQ ;

  /* Wait the acknowledge 等待硬件对CAN_MSR寄存器的INAK位置1来进行确认 */
  ...
  else 
  {
    ...
    /* Request leave initialisation 软件通过对CAN_MCR寄存器的INRQ位清0，来请求bxCAN退出初始化模式*/
    CANx->MCR &= ~MCR_INRQ;

   /* Wait the acknowledge */
   wait_ack = 0x00;

	 /*当硬件对CAN_MSR寄存器的INAK位清0就确认了初始化模式的退出*/
   while (((CANx->MSR & MSR_INAK) == MSR_INAK) && (wait_ack != INAK_TimeOut))
   {
     wait_ack++;
   }
